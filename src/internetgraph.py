...
def draw_graph(graph: asnutils.EdgeList, file: str, graph_layout="sfdp") -> List[Path]:
    ...
    for e in list(EdgeType):
        edges[e] = list(
            set((r.node1, r.node2) for r in filter(lambda x: x.edgeType == e, graph))
        )
    ...
    wrappers[EdgeType.ihost] = DrawWrapper(
        list(set(nodelist[EdgeType.ihost])),
        edges[EdgeType.ihost],
        "Indirect hosts",
        "o",
        textsize=3,
        nodesize=100,
    ...
    wrappers[EdgeType.host] = DrawWrapper(
        list(set(nodelist[EdgeType.host])),
        edges[EdgeType.host],
        "Hosts",
        "o",
        textsize=3,
    ...
    wrappers[EdgeType.start] = DrawWrapper(
        nodelist[EdgeType.start],
        edges[EdgeType.start],
        "Start",
        "p",
        textsize=4,
        nodesize=160,
    ...
    for country in countries:
        ASNs = set(
            t.node2
            for t in filter(
                lambda x: x.edgeType == EdgeType.cc and x.node1 == country, graph
            )
        )
        ...
        wrappers[country + "_ASN" if country else "unknown_ASN"] = DrawWrapper(
            ASNs,
            None,
            "ASN",
            "o",
            textsize=4,
            nodesize=300,
            color=get_color(index, index_tot),
        )
    ...
    wrappers[EdgeType.asn] = DrawWrapper(
        list(set(nodelist[EdgeType.asn])),
        edges[EdgeType.asn],
        "ASN",
        "o",
        textsize=4,
    ...
    wrappers[EdgeType.cc] = DrawWrapper(
        list(set(nodelist[EdgeType.cc])),
        edges[EdgeType.cc],
        "Countries",
        "d",
        textsize=5,
    ...
    wrappers[EdgeType.company] = DrawWrapper(
        list(set(nodelist[EdgeType.company])),
        edges[EdgeType.company],
        "Companies",
        "d",
        textsize=5,
        nodesize=400,
